module IF_Stage (
    ...
    input wire PCWrite,                  // Habilita a escrita no contador de programa (PC)
    //input wire MemRead,                  // Habilita a leitura da memória de instrução
    ...
    // Armazena o conjunto de instruções. O valor do PC é usado como endereço para buscar a instrução correspondente.
    // A leitura é habilitada pelo sinal 'MemRead'.
    //InstructionMemory instructionMemory (
    //    .address(currentPC),     // Endereço da instrução a ser buscada
    //    .MemRead(MemRead),       // Habilita a leitura da instrução
    //    .instruction(instruction) // Instrução lida da memória
    //);
